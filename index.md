...
to set start and end dates to be used by all other functions, as well as
tokens, user identifiers, and caching preferences. You can provide as
many or as few settings as you like. You can also pass the same as
parameters to individual functions, without using
[`cc_set()`](https://giocomai.github.io/cornucopia/reference/cc_set.md)
at all.
...
Things, however, may be not so simple, because of granular permissions.
In other words, you need to explicitly grant permissions to access
pages. As is characteristic of Facebook, they like to move settings
around, but you should be able to add this permission from the [Graph
Api Explorer](https://developers.facebook.com/tools/explorer/) (for
context, see also [this answer on
...
Data will be cached locally by default, assuming creatives will mostly
be added at the time when the ad is created.
...
as long as you have the `fb_page_id` of the Facebook page associated
with the given Instagram account and `fb_user_token` (notice, the user
token, not the page token). For reference, see also [step 5 of this
official
...
Responses to
[`cc_get_instagram_media()`](https://giocomai.github.io/cornucopia/reference/cc_get_instagram_media.md)
are cached by default, and updated with decreasing frequency as posts
get older (data are refreshed every day for the last week, once a week
for the last month, once a month for the last year, once a year for
previous years). As a consequence, you should mostly be able to keep
this in scripts and rely on it to autoaupdate data without much delay.
...
### Visualising campaign structure

If the campaign structure is not exceedingly complex, it may be possible
to visualise its structure giving it the shape of a flowchart. The
following function, previously included in the package and later dropped
in order to reduce dependencies, allows to achieve this simply by
passing to it an `ads_df` data frame as retrieved with
[`cc_get_fb_ads()`](https://giocomai.github.io/cornucopia/reference/cc_get_fb_ads.md).

In order to work, it requires the separate package `nomnomlgraph` which
