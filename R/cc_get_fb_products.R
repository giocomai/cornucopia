...
cc_get_fb_products <- function(
  ...
  if (is.null(fb_product_catalog_id)) {
    fb_product_catalog_id <- cc_get_settings(
    ...
    ({
      ...
      out_post[[i]] <- purrr::map(
        ...
        .f = function(current_item) {
          commerce_insights_df <- tibble::as_tibble(
            current_item$commerce_insights
          )
          current_item$commerce_insights <- NULL

          product_catalog_df <- tibble::as_tibble(
            current_item$product_catalog
          ) |>
            dplyr::rename(
          ...
          current_item$additional_image_urls <- list(
            current_item$additional_image_urls
          )

          current_item$image_cdn_urls <- list(current_item$image_cdn_urls)
          current_item$additional_image_cdn_urls <- list(
            current_item$additional_image_cdn_urls
          )

          current_item$capability_to_review_status <- list(
            purrr::map(
              current_item$capability_to_review_status,
              .f = function(x) tibble::as_tibble(x)
            ) |>
              purrr::list_rbind()
          )
