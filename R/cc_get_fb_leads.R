...
#' 
#'
#' @param form_id Identifier of the lead gen form, presumably an id of about 17 digits.
#' @param fields Defaults to `c("created_time", "id", "ad_id", "form_id", "field_data")`-
#' @param fb_page_token Takes precedence over `fb_user_token`. See documentation for required permissions.
#' @param fb_user_token Used as a fallback option if `fb_page_token` not given. See documentation for required permissions.
#' @param max_pages Maximum number of pages to retrieve (15 responses are included in each page). Defaults to 1000 pages internally.
...
cc_get_fb_leads <- function(form_id,
                            fields = c("created_time", "id", "ad_id", "form_id", "field_data"),
                            fb_page_token = NULL,
                            fb_user_token = NULL,
                            max_pages = NULL,
                            api_version = "v19.0") {
